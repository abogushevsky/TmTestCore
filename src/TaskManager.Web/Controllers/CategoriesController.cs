...
namespace TaskManager.Web.Controllers
{
    public class CategoriesController : TaskManagerApiControllerBase
    {
        ...
        public async Task<HttpResponseMessage> Get()
        {
            ...
            Category[] categories = await this._categoriesService.GetUserCategoriesAsync(currentUser.Id);
        ...
        {
            Category category = await this._categoriesService.GetCategoryById(id);
            if (category == null)
        ...
        public async Task<HttpResponseMessage> Post([FromBody] CategoryModel category)
        {
            ...
            int id = await this._categoriesService.AddCategoryAsync(category.ToCategory(await GetCurrentUser()));
        ...
        public async Task<HttpResponseMessage> Put([FromBody]CategoryModel category)
        {
            ...
            bool result = await this._categoriesService.UpdateCategoryAsync(category.ToCategory(await GetCurrentUser()));
            if (!result)
        ...
        {
            bool result = await this._categoriesService.DeleteCategoryAsync(id);
            if (!result)
